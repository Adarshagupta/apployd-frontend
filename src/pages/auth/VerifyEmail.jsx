...
const VerifyEmail = () => {
  ...
  const handleSendVerificationCode = async () => {
    try {
      ...
      // Switch to demo mode automatically if there's an API error
      setIsDemoMode(true);
      localStorage.setItem('neon_demo_mode', 'true');
      
      setStatus({
        type: 'warning',
        message: 'API error: Switched to demo mode. Use code 123456 to verify.'
      });
      
      toast({
        title: "API Error",
        description: "There was an error connecting to the verification service. Demo mode activated.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
